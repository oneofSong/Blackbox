...
int main(int argc, char *argv[])
{
	...
	struct winsize ws = get_winsize(); // window size(ws_col,ws_row)
	int win_index =0; // 현재까지  출력된 window 위치 
	...
	while((dent = readdir(dp)) != NULL) 
	{
		...
		{	win_index += strlen(dent->d_name);
			if(win_index < ws.ws_col){
				 printf("%s  ",dent->d_name);
				win_index += 2;
			}else{
				win_index = strlen(dent->d_name)+2;
				printf("\n");
				printf("%s  ",dent->d_name);
			}
		}
