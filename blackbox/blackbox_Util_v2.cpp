...
// time_difference
//
struct timeval time_differ(struct timeval start_time, struct timeval end_time)
{
	struct timeval result;
	if ((end_time.tv_usec - start_time.tv_usec) < 0)
	{
		result.tv_sec = end_time.tv_sec - start_time.tv_sec - 1;
		result.tv_usec = 1000000 - end_time.tv_usec - start_time.tv_usec;
	}
	else
	{
...
#ifdef DEBUG
	printf("thread finished : %u , start_time : %ld  \n", t_id, time(0));
#endif
